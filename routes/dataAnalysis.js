...
model.query("SELECT count(owner), owner From Public.\"dataAnalysis\" GROUP BY owner ORDER BY count(owner) DESC limit 7")
...
model.query("SELECT count(model) AS y, model AS label From Public.\"dataAnalysis\" GROUP BY model ORDER BY count(model) DESC LIMIT 5")
